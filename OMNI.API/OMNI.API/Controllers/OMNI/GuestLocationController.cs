    ...
    public class GuestLocationController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddEdit(GuestLocationModel model, CancellationToken cancellationToken)
        {
            ...
            if (model.Id > 0)
            {
                ...
                //data.GuestCategory = model.GuestCategory;
                data.Port = strPorts;
        ...
        [HttpDelete("{id:int}")]
        public async Task<GuestLocation> Delete([FromRoute] int id, CancellationToken cancellationToken)
        {
            GuestLocation data = await _dbOMNI.GuestLocation.Where(b => b.Id == id).FirstOrDefaultAsync(cancellationToken);
